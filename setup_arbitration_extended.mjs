...
async function createAndCompleteExpertProfile(token, body) {
...
async function createNeutralProfiles() {
    for (const u of neutralUsers) {
        ...
        if (u.expertProfileBody) {
            ...
            await createAndCompleteExpertProfile(neutral.token, expertBody);
...
async function submitExpertCriteria(token, caseId, sessionId, criteria) {
    const res = await fetch(
        `${baseCase}/${caseId}/arbitration/${sessionId}/expert-criteria`,
        {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
...
async function getNeutralEmailByProfileId(profileId) {
...
async function setupArbitrationExtended() {
    ...
    const arbitrationOptions = {
        arbitrationOptionType: 'extended',
        paperOnly: false,
        expertOnPanel: true,
    ...
    console.log('✅ Claimant submitted arbitration options (extended + expert)');
    ...
    console.log('✅ Respondent submitted arbitration options (extended + expert)');

    const arbitratorCriteria = {
    ...
    await submitArbitrationCriteria(
        claimantToken,
        caseId,
        sessionId,
        arbitratorCriteria,
    );
    console.log('✅ Claimant submitted arbitrator criteria');
    ...
    console.log('✅ Respondent submitted arbitrator criteria');

    const expertCriteria = {
        isNeutralFormerJudge: false,
        minYearsOfExperience: 3,
        areaOfExpertise: ['06f4e277-3f27-48d9-915c-2c212daa49c8'],
        targetHourlyRate: 600,
        submitterFullName: 'Jure Pavlovic',
    };

    await submitExpertCriteria(claimantToken, caseId, sessionId, expertCriteria);
    console.log('✅ Claimant submitted expert criteria');

    await submitExpertCriteria(respondentToken, caseId, sessionId, expertCriteria);
    console.log('✅ Respondent submitted expert criteria');

    await new Promise((resolve) => setTimeout(resolve, 1000));

    const arbitratorNeutrals = await getSystemNeutrals(
        claimantToken,
        caseId,
        sessionId,
        'arbitrator',
    ...
    console.log(
        `✅ Retrieved ${arbitratorNeutrals.length} system neutrals for arbitrator role`,
    );

    const arbitratorNeutralIds = arbitratorNeutrals.map((n) => n.id);

    await submitNeutralPreferences(
        claimantToken,
        caseId,
        sessionId,
        arbitratorNeutralIds,
        'Jure Pavlovic',
        'arbitrator',
    );
    console.log('✅ Claimant submitted arbitrator neutral ordering');
    ...
    console.log('✅ Respondent submitted arbitrator neutral ordering');

    const expertNeutrals = await getSystemNeutrals(
        claimantToken,
        caseId,
        sessionId,
        'expert',
    ...
    console.log(
        `✅ Retrieved ${expertNeutrals.length} system neutrals for expert role`,
    );

    const expertNeutralIds = expertNeutrals.map((n) => n.id);
    ...
    console.log('✅ Claimant submitted expert neutral ordering');
    ...
    console.log('✅ Respondent submitted expert neutral ordering');

    await new Promise((resolve) => setTimeout(resolve, 500));

    const updatedSession = await getArbitrationSession(claimantToken, caseId);
    const selectedNeutrals = updatedSession.selectedNeutrals?.filter(
        (n) => n.status === 'accepted',
    );

    if (!selectedNeutrals || selectedNeutrals.length === 0) {
        console.error('❌ No accepted neutrals found after ordering');
        process.exit(1);
    }

    console.log(`✅ ${selectedNeutrals.length} neutrals selected`);

    const processedNeutrals = [];

    for (const selectedNeutral of selectedNeutrals) {
        let neutralUserEmail = selectedNeutral.email;

        if (!neutralUserEmail) {
            neutralUserEmail = await getNeutralEmailByProfileId(
                selectedNeutral.profileId,
            );
        }

        if (!neutralUserEmail) {
            console.error(
                `❌ Could not find email for neutral with profile ${selectedNeutral.profileId}`,
            );
            continue;
        }

        console.log(
            `✅ Processing neutral: ${neutralUserEmail} (role: ${selectedNeutral.role || 'unknown'})`,
        );
        ...
        console.log(`✅ Neutral ${neutralUserEmail} accepted case invite`);

        await submitConflictCheck(neutral.token, caseId, 'no_conflict');
        console.log(`✅ Neutral ${neutralUserEmail} submitted conflict check (no conflict)`);

        processedNeutrals.push({
            email: neutralUserEmail,
            role: selectedNeutral.role,
            profileId: selectedNeutral.profileId,
        });
    }
    ...
    console.log('\n========================================');
    console.log('EXTENDED PANEL ARBITRATION SETUP COMPLETE');
    console.log('========================================');
    console.log(`Case ID: ${caseId}`);
    console.log(`Session ID: ${sessionId}`);
    console.log(`Panel Type: extended`);
    console.log(`Expert on Panel: true`);
    console.log(`Selected Neutrals (${processedNeutrals.length}):`);
    for (const n of processedNeutrals) {
        console.log(`  - ${n.email} (${n.role || 'role unknown'})`);
    }
...
await setupArbitrationExtended().catch((e) => {
    console.error('❌ Error:', e?.message || e);
    process.exit(1);
});
